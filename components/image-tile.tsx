...
export function ImageTile({ title, description, image, onClick, previewUrl }: ImageTileProps) {
  const handleClick = () => onClick(image);

  return (
    <li className="group relative w-72 h-72" onClick={handleClick} role="presentation">
      <Image src={previewUrl} alt={title} fill className="object-cover border-2" />
